...
export class CreatePaymentAction {
    ...
    private async validateCurrencyAndWalletsAndCreateDebit(
        ...
        if (!(await debitWallet.save())) {
            throw new BadGatewayException(`Failed to initiate payment debit for ${debitWallet.id}`)
        }
    }
    ...
    private async validateDebitWalletBalance(payment: InitiatePaymentDto) {
        const debitWallet = await this.walletModel.findById(payment.debitWallet);
    ...
    private async initiateDebit(payment: any) {
